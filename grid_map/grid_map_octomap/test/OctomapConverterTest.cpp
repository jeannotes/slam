...
TEST(OctomapConversion, convertOctomapToGridMap) {
    ...
    // Convert to grid map.
    GridMap gridMap;
    bool res = GridMapOctomapConverter::fromOctomap(octomap, "elevation", gridMap);
    ASSERT_TRUE(res);
...
TEST(OctomapConversion, convertOctomapToGridMapWithBoundingBox) {
    ...
    bool res = GridMapOctomapConverter::fromOctomap(octomap, "elevation", gridmap, &minpt, &maxpt);
    ASSERT_TRUE(res);
    Length length = gridmap.getLength();
    EXPECT_FLOAT_EQ(1.5, length.x());
    EXPECT_FLOAT_EQ(1.5, length.y());
    EXPECT_FLOAT_EQ(octomap.getResolution(), gridmap.getResolution());
    EXPECT_FLOAT_EQ(0.5 * octomap.getResolution(), gridmap.atPosition("elevation", Position(1.0, 1.0)));
}
