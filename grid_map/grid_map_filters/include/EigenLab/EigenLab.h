...
public:
    ...
    inline void setShared(const Derived & mat) {
        setShared(mat.data(), mat.rows(), mat.cols());
    }
    ...
    Value(const Value & val) : mLocal(1, 1), mShared(mLocal.data(), mLocal.rows(), mLocal.cols()) {
        (* this) = val;
    }
...
public:
    ...
    inline bool cacheExpressions() const {
        return mCacheChunkedExpressions;
    }
    inline void setCacheExpressions(bool b) {
        mCacheChunkedExpressions = b;
    }
    inline void clearCachedExpressions() {
        mCachedChunkedExpressions.clear();
    }
...
private:
    ...
    inline bool isVariable(const std::string & name) const {
