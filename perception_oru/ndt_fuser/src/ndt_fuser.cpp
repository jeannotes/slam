...
void NDTFuser::initialize(Eigen::Affine3d initPos,
                          pcl::PointCloud< pcl::PointXYZ >& cloud) {
    std::cout << "Initializing NDT "
              "FUSER!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n";
    ...
    if (be2D) {
        if (matcher2D.match(*map, ndlocal, Tinit, true)) {
            ...
            if ((diff.translation().norm() > max_translation_norm || diff.rotation().eulerAngles(0, 1, 2).norm() > max_rotation_norm) && checkConsistency) {
                fprintf(stderr, "****  NDTFuser -- ALMOST DEFINATELY A "
                ...
                if (diff_fuse.translation().norm() > translation_fuse_delta || diff_fuse.rotation().eulerAngles(0, 1, 2).norm() > rotation_fuse_delta) {
                    ...
                    t4 = getDoubleTime();
                    map->computeNDTCells(CELL_UPDATE_MODE_SAMPLE_VARIANCE, 1e5,
                                         255, spose.translation(), 0.1);
                    Tlast_fuse = Tnow;
                    if (visualize) {
#ifndef NO_NDT_VIZ
                        viewer->plotNDTSAccordingToOccupancy(-1, map);
            ...
            /*
                   std::cout<<"load: "<<t2-t1<<" match: "<<t3-t2<<" fuse:
               "<<t4-t3<<" total: "<<t4-t1<<std::endl; FILE *ftmp =
               fopen("tmp.txt","a"); fprintf(ftmp,"%lf, ",t4-t3); fclose(ftmp);
             */
        } else {
            Tnow = Tnow * Tmotion;
        }
        ...
        if (matcher.match(*map, ndlocal, Tinit, true)) {
            ...
            if ((diff.translation().norm() > max_translation_norm || diff.rotation().eulerAngles(0, 1, 2).norm() > max_rotation_norm) && checkConsistency) {
                ...
                if (diff_fuse.translation().norm() > translation_fuse_delta || diff_fuse.rotation().eulerAngles(0, 1, 2).norm() > 2 * rotation_fuse_delta) {
                    t3 = getDoubleTime();
                    map->addPointCloud(spose.translation(), cloud, 0.06, 2.5);
                    t4 = getDoubleTime();
                    map->computeNDTCells(CELL_UPDATE_MODE_SAMPLE_VARIANCE, 1e5,
            ...
            std::cout << "load: " << t2 - t1 << " match: " << t3 - t2
                      << " fuse: " << t4 - t3 << " total: " << t4 - t1
                      << std::endl;
