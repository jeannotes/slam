...
bool NDTMapHMT::tryLoadPosition(const Eigen::Vector3d& newPos)
{
    ...
    while (getline(&line, &len, meta_f) > 0) {
        ...
        if (fabsf(newPos(0) - cen.x) < sizeX / 2 && fabsf(newPos(1) - cen.y) < sizeY / 2 && fabsf(newPos(2) - cen.z) < sizeZ / 2) {
            found = true;
            centerX = cen.x;
            centerY = cen.y;
            centerZ = cen.z;
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (grid_[i][j] == NULL)
                    return -1;
                grid_[i][j]->getCenter(centx, centy, centz);
                if (fabsf(cen.x - centx) < epsilon && fabsf(cen.y - centy) < epsilon && fabsf(cen.z - centz) < epsilon) {
                    already[i][j] = true;
...
bool NDTMapHMT::tryLoad(const double& cx, const double& cy, const double& cz, LazyGrid*& grid)
{
    ...
    while (getline(&line, &len, meta_f) > 0) {
        ...
        if (fabsf(cen.x - cx) < epsilon && fabsf(cen.y - cy) < epsilon && fabsf(cen.z - cz) < epsilon) {
            token[strlen(token) - 1] = '\0';
            jffname = token;
            found = true;
            //std::cerr<<"file already for "<<jffname<<std::endl;
