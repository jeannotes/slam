...
//	imufile.att0 = CVect3(9.005593e-01, 1.676776e-01, -9.059221e+01)*glv.deg;  // true attitude
	...
	for(int i=0; i<2000/imufile.ts; i+=nn)
	{
		...
		{
			for(int j=0; j<nn; j++) { wmm = wmm+wm[j]; vmm = vmm+vm[j]; }
		...
		else
		{
			...
			if(i<500/imufile.ts)	// fine align
			{
				...
				if(i%100==0)
				{
					...
					kf.MeasUpdate();
					sins.qnb = sins.qnb - *phi;  *phi = CVect3(0.0);
					sins.vn = sins.vn - *dvn;    *dvn = CVect3(0.0);
					sins.pos = sins.pos - *dpos; *dpos = CVect3(0.0);  // height feedback
